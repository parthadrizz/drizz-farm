...
func RegisterRoutes(r chi.Router, cfg *config.Config, p *pool.Pool, b *session.Broker, lic *license.Validator, deps ServerDeps) {
	...
	r.Route("/api/v1", func(r chi.Router) {
		...
		r.Post("/sessions/{id}/file/push", devH.PushFile)       // local_path → device (host-local)
		r.Post("/sessions/{id}/file/pull", devH.PullFile)       // device → local_path (host-local)
		r.Post("/sessions/{id}/files/upload", devH.UploadFile)  // multipart → device (CI-friendly)
